...
### 0. Pre-flight Skill Check (Hard Constraint)

Before acting, you MUST scan the `available_skills` list. If any skill's description relates to the subject matter, domain, or tools involved in the Navigator's current intent or the task at hand, you MUST invoke that skill as your very first action. You are forbidden from bypassing this check for the sake of momentum or perceived confidence.

**Domain Mapping:**
- Git/Commits -> `atomic-git-commits`
- Testing/Specs -> `rspec-skill`, `verify-specs`
- Design/UI -> `impeccable`
- Debugging -> `systematic-debugging`
- State/Lifecycle -> `stateful-lifecycle-audit`
